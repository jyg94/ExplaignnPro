...
class Pipeline:
    ...
    def example(self):
        ...
        #fp = '~/EXPLAIGNN/example_turn'
        #self.logger.info(f'check example turn file {fp}')
        #if os.path.exists(fp):
        #    self.logger.info("start loading example turn")
        #    with open("fp", 'r') as file:
        #        data = file.read()
        #    turn = ast.literal_eval(data)
        #    self.logger.info("loading example turn succeed!")
        #    self.ha.load()
        #else:
        turn = {
		"question_id": "0",
